...
final class DeepWaterGerstnerShape extends WaveShape {
  ...
  Offset computeOffset({
    ...
    final gerstnerVertical =
        baseVerticalDisp * dispersionFactor + (horizontalDisp * 0.25);
...
final class StormGerstnerShape extends WaveShape {
  ...
  Offset computeOffset({
    ...
    final turbulence1 =
        turbulence * amplitude * 0.3 * sin(k * x * 2.1 + phase * 1.3);
    final turbulence2 =
        turbulence * amplitude * 0.2 * sin(k * x * 3.7 + phase * 0.7);
    final turbulence3 =
        turbulence * amplitude * 0.15 * cos(k * x * 1.9 + phase * 2.1);

    // Foam effect on wave crests (higher amplitude near peaks)
    final foamHeight = foamFactor *
        amplitude *
        max(0, sin(theta).clamp(0.7, 1.0) - 0.7) *
        3.33;

    // Combine all effects
    final totalTurbulence = turbulence1 + turbulence2 + turbulence3;
    final stormVertical = baseVerticalDisp +
        (horizontalDisp * 0.4) +
        totalTurbulence +
        foamHeight;
...
final class ShallowGerstnerShape extends WaveShape {
  ...
  Offset computeOffset({
    ...
    final shallowVertical =
        baseVerticalDisp * nonlinearFactor + (horizontalDisp * 0.2);
